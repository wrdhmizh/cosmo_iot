...
// Redirect if already logged in as private user
if (isset($_SESSION['user_id'], $_SESSION['user_type']) && $_SESSION['user_type'] === 'private') {
    header('Location: private_dashboard.php');
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($username === '' || $password === '') {
        ...
        $stmt = $pdo->prepare("SELECT id, password FROM users WHERE username = :username AND user_type = 'private' LIMIT 1");
        $stmt->execute([':username' => $username]);
        $user = $stmt->fetch(PDO::FETCH_ASSOC);

        if ($user && password_verify($password, $user['password'])) {
            $_SESSION['user_id'] = $user['id'];
            $_SESSION['user_type'] = 'private';
...
<body>
    <div class="login-card">
        ...
        <div class="signup-link">
            Don't have an account? <a href="private_signup.php">Sign Up</a>
        </div>
    </div>
</body>
</html>

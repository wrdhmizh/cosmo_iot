...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($username === '' || $password === '' || $signup_token === '') {
        echo "<script>alert('Please fill in all fields.'); window.location.href='public_signup.php';</script>";
        exit();
    }

    $stmt = $pdo->prepare("SELECT * FROM public.signup_tokens WHERE token = :token AND is_used = FALSE AND user_type = 'public' AND (expiry IS NULL OR expiry >= CURRENT_DATE) LIMIT 1");
    $stmt->execute([':token' => $signup_token]);
    $token_row = $stmt->fetch(PDO::FETCH_ASSOC);

    if (!$token_row) {
        echo "<script>alert('Invalid, used or expired PUBLIC token.'); window.location.href='public_signup.php';</script>";
        exit();
    }
    ...
    if ($stmt->fetch()) {
        echo "<script>alert('Username already exists.'); window.location.href='public_signup.php';</script>";
        exit();
    }
    ...
    echo "<script>alert('Public signup successful. You can now login.'); window.location.href='login.php';</script>";
    exit();
}
?>
...
<div id="modalOverlay" class="modal-overlay">
    <div class="modal">
        ...
        <div class="request-token">
            Need one? 
            <a href="mailto:dreamteams666666@gmail.com?subject=Request%20for%20Signup%20Token&body=Hi,%20I%20would%20like%20to%20request%20a%20signup%20token." target="_blank">
                Request Token
            </a>
        </div>
    </div>
